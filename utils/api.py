    ...
    def _apply_image(self, pic):
        ...
        if npimg.shape[2] == 1:
            ...
            if self.mode is not None and self.mode not in permitted_2_channel_modes:
                raise ValueError("Only self.modes {} are supported for 2D inputs".
            ...
            if self.mode is not None and self.mode not in permitted_4_channel_modes:
                raise ValueError("Only self.modes {} are supported for 4D inputs".
            ...
            if self.mode is not None and self.mode not in permitted_3_channel_modes:
                raise ValueError("Only self.modes {} are supported for 3D inputs".
...
try:
    ...
    def torch2paddle(model_path, model):
        model_base_path = '/app/wht/lc/VisionLAN/pytorch_VisionLAN//output/'
        torch_model_path = model_base_path + model_path
        paddle_model_path = model_base_path + model_path
        ...
        torch_dict = torch.load(torch_model_path)
        paddle_dict = {}
        paddle_dict['state_dict'] = {}

        write_dict(model.state_dict(), model_base_path + '/paddleParams.txt')
        write_dict(torch_dict, model_base_path + '/torchParams.txt')

        paddle_dict['state_dict'] = rename_state_dict(model.state_dict(), torch_dict)
        write_dict(paddle_dict['state_dict'], model_base_path + '/rename_Params.txt')
...
def clip_grad_norm_(
    ...
    for p in parameters:
        p.grad.detach().set_value(paddle.multiply(p.grad.detach(), clip_coef_clamped))
