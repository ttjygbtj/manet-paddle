...
class Added_BCEWithLogitsLoss(nn.Layer):
    ...
    def forward(self,dic_tmp,y,step):
        ...
        # to loss_collection and set reduction = None to keep the shape.
                ...
                _,top_k_indices = paddle.topk(pixel_losses,k=top_k_pixels,dim=2)
    ...
    def forward(self,dic_tmp,y,step):
        ...
        # to loss_collection and set reduction = None to keep the shape.
                ...
                if self.hard_example_mining_step==0:
                    ...
                    ratio = min(1.0,step/float(self.hard_example_mining_step))
                    top_k_pixels = int((ratio*self.top_k_percent_pixels+(1.0-ratio))*num_pixels)
                top_k_loss,top_k_indices = paddle.topk(pixel_losses,k=top_k_pixels,dim=1)
...
#            self.celoss = nn.CrossEntropyLoss(ignore_index=255,reduction='none')
    ...
    def forward(self,pred_logits,gts,step):
        pos_num = paddle.sum(gts == 1, dtype=paddle.float)
        neg_num = paddle.sum(gts == 0, dtype=paddle.float)

        weight_pos = neg_num / (pos_num + neg_num)
        weight_neg = pos_num / (pos_num + neg_num)
        weights = paddle.tensor([weight_neg, weight_pos])
        if self.top_k_percent_pixels==None:
            sig_pred_logits=paddle.sigmoid(pred_logits)
#            self.celoss=nn.CrossEntropyLoss(weight=weights.cuda(),ignore_index=255,reduction='mean')
            ...
            if paddle.sum(gts)==0:
                ...
                dcloss = (paddle.sum(sig_pred_logits*sig_pred_logits)+paddle.sum(gts*gts))/(paddle.sum(2*sig_pred_logits*gts)+1e-5)
