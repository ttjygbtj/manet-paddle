...
class Manager(object):
    ...
    def train(self, damage_initial_previous_frame_mask=True, lossfunc='cross_entropy', model_resume=False):
        ...
        ########
        optimizer = optim.SGD(param_list, lr=cfg.TRAIN_LR, momentum=cfg.TRAIN_MOMENTUM,
                              weight_decay=cfg.TRAIN_WEIGHT_DECAY, nesterov=True)
        ...
        while step < cfg.TRAIN_TOTAL_STEPS:
            ...
            for ii, sample in enumerate(trainloader):
                ...
                tmp_dic = self.model(inputs, ref_scribble_labels, label1s, use_local_map=True, seq_names=seq_names,
                                     gt_ids=obj_nums, k_nearest_neighbors=cfg.KNNS)
                ...
                optimizer.zero_grad()
                loss.backward()
                paddle.nn.utils.clip_grad_norm_(self.model.parameters(), cfg.TRAIN_CLIP_GRAD_NORM)
                optimizer.step()
                ...
                if step % 50 == 0:
                    ...
                    print('step:{},now_lr:{} ,loss:{:.4f}({:.4f})'.format(step, now_lr, running_loss.val,
                                                                          running_loss.avg))
    ...
    def test_VOS(self, use_gpu=True):
        ...
        with paddle.no_grad():
            for seq_name in seqs:
                ...
                test_dataset = DAVIS2017_VOS_Test(root=cfg.DATA_ROOT, transform=tr.ToTensor(),
                                                  result_root=cfg.RESULT_ROOT, seq_name=seq_name)
                ...
                for ii, sample in enumerate(test_dataloader):
                    ...
                    inputs = paddle.cat((ref_img, prev_img, current_img), 0)
                    if use_gpu:
                        inputs = inputs.cuda()
                    ...
                    ref_frame_embedding, previous_frame_embedding, current_frame_embedding = paddle.split(tmp,
