...
from paddlevideo.loader.builder import build_custom_dataloader, build_sampler, build_pipeline
from paddlevideo.solver import build_lr, build_optimizer
from paddlevideo.utils import get_logger, load
import paddle.distributed as dist
import os.path as osp
from paddlevideo.utils import (build_record, log_batch, log_epoch, save, mkdir)
    ...
    def train_step(self,
        ...
        while step < max_iters:
            ...
            for r in range(round_):
                ...
                sampler = AttrDict({"name": "RandomIdentitySampler"})
                    ...
                    for ii in range(start_annotated_frame + 1, total_frame_num):
                        ...
                        if is_save_image:
                            ...
                            if not os.path.exists(inter_file_path):
                                os.makedirs(inter_file_path)
                            im.save(
                                os.path.join(inter_file_path, imgname + '.png'))
                    #######################################
